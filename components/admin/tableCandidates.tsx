...
const Candidates = ({ candidates, onCandidatesUpdate }: Props) => {
    ...
    setIsDeleting(true)
    try {
      const response = await fetch(`${config.env.apiEndpoint}/api/candidates/deleteCanditates/${id}`, {
        method: 'DELETE',
      })
          ...
          <Table>
            ...
            <TableBody>
              {candidates.map((candidate) => (
                <TableRow key={candidate.id} className="border-yellow-500/20 hover:bg-gray-100">
                  <TableCell>
                    <Image
                      src={candidate.image}
                      alt={candidate.nom}
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        ...
                        title="Êtes-vous sûr ?"
                        description={`Cette action ne peut pas être annulée. Cela supprimera définitivement le candidate "${candidate.nom+" "+candidate.prenom}"`}
